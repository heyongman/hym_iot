...
char oled_re_show_str(uint16_t x, uint16_t y,char* str, FontDef_t* Font, SSD1306_COLOR_t color){
    uint8_t len =  128/Font->FontWidth;
    char blank[len];
    ...
    oled_show_str(x, y,blank , Font, color);
	return oled_show_str(x, y, str, Font, color) ;
}
