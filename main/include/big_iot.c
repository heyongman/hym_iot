...
* @brief        定义接收以及处理贝壳物联命令的相关函数
* @details      获取得到贝壳物联的控制命令并控制
* @author       Helon_Chan 
* @par Copyright (c):  
*               红旭无线开发团队
* @par History:          
...
static void json_parse(const char *json_data)
{
  ...
  else
  {
    ...
    switch (strcmp(method, "\"checkinok\"") == 0 ? SIGN_IN_OK : strcmp(method, "\"token\"") == 0 ? GET_TOKEN : strcmp(method, "\"say\"") == 0 ? RECEVICE : strcmp(method, "\"WELCOME TO BIGIOT\"") == 0 ? TCP_CONNECT_SUCCESS : INVALID_METHOD)
    {
    /* ESP32成功连接上云平台 */
